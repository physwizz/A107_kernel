									...
									size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if(temp_buf == NULL)
		{
			E("%s, Allocate temp_buf failed!\n", __func__);
			goto ALOC_TEMP_BUF_FAIL;
		}
...
static struct wt_sec_cmd himax_commands[] = {
...
#ifdef SEC_TSP_FACTORY_TEST
		val = wt_sec_cmd_init(&private_ts->sec, himax_commands,ARRAY_SIZE(himax_commands), WT_SEC_CLASS_DEVT_TSP);
		if (val < 0) {
			E("%s: Failed to wt_wt_sec_cmd_init\n", __func__);
			return -ENODEV;
		}
#endif
...
#ifdef SEC_TSP_FACTORY_TEST
	wt_sec_cmd_exit(&private_ts->sec, WT_SEC_CLASS_DEVT_TSP);
#endif
